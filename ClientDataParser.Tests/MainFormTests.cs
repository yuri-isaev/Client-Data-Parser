...
public class MainFormTests
{
  ...
  public void LoadClients_Should_Load_Clients_Successfully()
  {
    ...
    _mockClientRepository
      ...
      .Returns(clients); // Убеждаемся, что это значение не равно нулю

    // Act
    _mainForm.LoadClients();

    // Assert
    Assert.IsNotNull(_mainForm._clientsBindingList); // Утверждаем, что он инициализирован
  ...
  public void LoadClients_Should_Initialize_With_Empty_List_When_No_Clients()
  {
    ...
    _mockClientRepository
      .Setup(repo => repo.GetAllClientsOrderedByLastName())
      .Returns((List<Client>) null); // Имитировать возврат null
    ...
    // Assert
    Assert.IsNotNull(_mainForm._clientsBindingList);         // Утверждаем, что он инициализирован
    Assert.AreEqual(0, _mainForm._clientsBindingList.Count); // Утверждаем, что счетчик равен 0
  }
